...
export class MinecraftEC2Wrapper {
	...
	public async startInstance(): Promise<boolean> {
		if (!await MinecraftEC2Wrapper.SERVER_DETAILS.isServerRunning() || !await this.isInstanceUp()) {
			const runInstancesResponse = await MinecraftEC2Wrapper.EC2_CLIENT.send(new RunInstancesCommand({
	...
	public async stopInstance(): Promise<boolean> {
		if (await MinecraftEC2Wrapper.SERVER_DETAILS.isServerRunning() || await this.isInstanceUp()) {
			...
			await MinecraftEC2Wrapper.EC2_CLIENT.send(new StopInstancesCommand({
			...
			const terminateInstancesResult = await MinecraftEC2Wrapper.EC2_CLIENT.send(new TerminateInstancesCommand({
				InstanceIds: [instanceId]
			}));
	...
	private async waitForAMIToBeCreated(): Promise<void> {
		...
		do {
			...
			response = await MinecraftEC2Wrapper.EC2_CLIENT.send(new DescribeImagesCommand({
				ExecutableUsers: ['self'], Filters: [{
					Name: 'state', Values: ['pending', 'failed', 'error']
				}]
			}));
	...
	private async waitForSnapshotToBeCreated(): Promise<void> {
		...
		do {
			...
			response = await MinecraftEC2Wrapper.EC2_CLIENT.send(new DescribeSnapshotsCommand({
	...
	private async getNewestSnapshot(): Promise<string> {
		const response = await MinecraftEC2Wrapper.EC2_CLIENT.send(new DescribeSnapshotsCommand({
			OwnerIds: [MinecraftEC2Wrapper.AWS_ACCOUNT_ID.replace(/-/g, '')]
		}));
	...
	private async deleteOldAmi(oldAMIid: string, oldSnapshotId: string): Promise<void> {
		await MinecraftEC2Wrapper.EC2_CLIENT.send(new DeregisterImageCommand({
			ImageId: oldAMIid
		}));
		await MinecraftEC2Wrapper.EC2_CLIENT.send(new DeleteSnapshotCommand({
			SnapshotId: oldSnapshotId
		}));
	}

	private async makeAMI(instanceId: string): Promise<string> {
		const createImageResponse = await MinecraftEC2Wrapper.EC2_CLIENT.send(new CreateImageCommand({
			InstanceId: instanceId, Name: MinecraftEC2Wrapper.AMI_NAME + '-' + Date.now().toString(),
		}));
	...
	public async rebootInstance(): Promise<void> {
		if (await MinecraftEC2Wrapper.SERVER_DETAILS.isServerRunning()) {
			const result = await MinecraftEC2Wrapper.EC2_CLIENT.send(new RebootInstancesCommand({
	...
	public async getInstanceDomainName(): Promise<string> {
		const instanceId = await MinecraftEC2Wrapper.SERVER_DETAILS.getInstanceId();
		const response = await MinecraftEC2Wrapper.EC2_CLIENT.send(new DescribeInstancesCommand({
			InstanceIds: [instanceId]
		}));
