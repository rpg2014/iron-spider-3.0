...
const DateCard: React.FC<DateCardProps> = ({ date, onClickNav = false }) => {
  ...
  const formatDate = (date: Date | undefined) => {
    ...
    console.log(`converted? date unix time: ${date.getTime()}`);
  ...
  const RenderedCard = () => (
    <Card className={`${styles.cardContainer} hover:shadow-lg transition-shadow duration-300`}>
      <CardHeader className="space-y-2">
        <CardTitle className="text-2xl font-bold">{date.title}</CardTitle>
        <div className="flex flex-col space-y-1">
          <div className="flex items-center space-x-2">
            <MapPinIcon size={ICON_SIZE} className="w-4 h-4 text-gray-400 flex-shrink-0" />
          ...
          <div className="flex items-center space-x-2">
            <CalendarIcon size={ICON_SIZE} className="w-4 h-4 text-gray-400 flex-shrink-0" />
            {/* <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          {date.dateThrower && (
            <div className="flex items-center space-x-2">
              <UserRoundIcon size={ICON_SIZE} className="w-4 h-4 text-gray-400 flex-shrink-0" />
      ...
      <CardContent className="space-y-4">
        {date.pictureId && (
          <div className="relative">
            {!imgLoaded && <Skeleton className="w-full h-auto aspect-square rounded" />}
            <img
              src={date.pictureId}
              alt={`Date at ${date.location}`}
              onLoad={() => setImgLoaded(true)}
              style={{ height: !imgLoaded ? "0px" : undefined }}
              className={`w-full h-auto rounded object-cover ${imgLoaded ? "" : "hidden"}`}
            />
          </div>
        )}
        {date.note && (
          <div className="flex items-start space-x-2">
            <BookHeartIcon size={ICON_SIZE} className="w-4 h-4 text-gray-400 mt-1 flex-shrink-0" />
  ...
  return (
    <>
      {onClickNav ? (
        <Link to={`/dates/${date.id}`} className="block cursor-pointer">
          <RenderedCard />
        </Link>
      ) : (
        <RenderedCard />
