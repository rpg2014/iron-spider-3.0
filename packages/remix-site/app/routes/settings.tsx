...
export default function Settings() {
    ...
    return <main className={styles.settingsContainer}>
        <h1>Settings</h1>
        <hr />

        <div className={styles.setting}>
            <label className={styles.settingsLabel}>Notifications</label>
            <button disabled={notificationPermission === "granted"} className={styles.button} onClick={() => {
        ...
        <div className={styles.setting}>
            ...
            <button className={styles.button} disabled={notificationPermission === undefined || notificationPermission !=='granted'} onClick={async () => {
                if (notificationPermission === "granted") {
                    if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
                        await navigator.serviceWorker.ready.then((reg: ServiceWorkerRegistration) => reg.showNotification("Test Notification", {
