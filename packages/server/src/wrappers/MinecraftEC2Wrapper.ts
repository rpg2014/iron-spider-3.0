...
export class MinecraftEC2Wrapper {
    ...
    if (!(await MinecraftEC2Wrapper.SERVER_DETAILS.isServerRunning()) || !(await this.isInstanceRunning())) {
      ...
      try {
        ...
        throw new InternalServerError({
          message: `Running the instance threw error ${JSON.stringify(e)} with input ${JSON.stringify(runInstancesCommandInput)}`,
        });
      }
  ...
  public async stopInstance(): Promise<boolean> {
    if ((await MinecraftEC2Wrapper.SERVER_DETAILS.isServerRunning()) || (await this.isInstanceRunning())) {
      ...
      try {
        ...
        throw new InternalServerError({
          message: `Terminate Instance threw error ${JSON.stringify(e)} with input ${JSON.stringify(terminateInstancesCommandInput)}`,
        });
      }
  ...
  public async isInstanceInState(status: EC2State): Promise<boolean> {
    ...
    if (isInState) {
      console.info(`Server Instance ${await MinecraftEC2Wrapper.SERVER_DETAILS.getInstanceId()} is ${EC2State[status]}`);
    }
    return isInState;
  }
